...
async function handleMensagem(sock, msg) {
  ...
  if (pedido.etapa === 'aguardando_nome') {
    pedido.nome_completo = text;
    await sock.sendMessage(from, {
      text: `Obrigado, ${pedido.nome_completo}! Agora, por favor, digite seu CPF (somente números).`,
    });
  ...
  if (pedido.etapa === 'aguardando_cpf') {
    ...
    try {
      ...
      const response = await axios.post(
        'http://0.0.0.0:5678/webhook-test/cliente-upsert',
        {
      ...
      if (response.data.clienteExiste) {
        await sock.sendMessage(from, {
          text: `Olá novamente, ${pedido.nome_completo}! Parece que você já esteve aqui.`,
        });
      } else {
        await sock.sendMessage(from, {
          text: `Olá, ${pedido.nome_completo}! Seja bem-vindo(a) pela primeira vez!`,
        });
      }
