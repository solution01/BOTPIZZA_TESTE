...
function processStreamChunk(streamMode, chunk) {
    if (streamMode === 'updates') {
        ...
        if (agentChunk?.create_plan?.workflowPlan) {
            ...
            const lastMessage = agentChunk.create_plan.messages[agentChunk.create_plan.messages.length - 1];
            const messageChunk = {
                role: 'assistant',
                type: 'message',
                text: lastMessage.content,
        ...
        if (agentChunk?.adjust_plan?.workflowPlan) {
            const workflowPlan = agentChunk.adjust_plan.workflowPlan;
            const planChunk = {
                role: 'assistant',
                type: 'plan',
                plan: workflowPlan.plan,
...
function createToolCallMessage(toolCall, builderTool) {
    if (toolCall.name === 'generate_workflow_plan') {
        const workflowPlan = toolCall.args;
        return {
            role: 'assistant',
            type: 'plan',
            plan: workflowPlan.plan,
