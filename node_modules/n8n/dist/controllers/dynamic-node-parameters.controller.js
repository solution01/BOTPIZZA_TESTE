...
let DynamicNodeParametersController = class DynamicNodeParametersController {
    ...
    async getOptions(req, _res, payload) {
        ...
        if (projectId) {
            if (await (0, check_access_1.userHasScopes)(req.user, ['dataStore:listProject'], false, { projectId })) {
    ...
    async getLocalResourceMappingFields(req, _res, payload) {
        ...
        const additionalData = await (0, workflow_execute_additional_data_1.getBase)(req.user.id, currentNodeParameters);
        return await this.service.getLocalResourceMappingFields(methodName, path, additionalData, nodeTypeAndVersion);
    }
    async getActionResult(req, _res, payload) {
        const { currentNodeParameters, nodeTypeAndVersion, path, credentials, handler, payload: actionPayload, } = payload;
        const additionalData = await (0, workflow_execute_additional_data_1.getBase)(req.user.id, currentNodeParameters);
        return await this.service.getActionResult(handler, path, additionalData, nodeTypeAndVersion, currentNodeParameters, actionPayload, credentials);
    }
};
