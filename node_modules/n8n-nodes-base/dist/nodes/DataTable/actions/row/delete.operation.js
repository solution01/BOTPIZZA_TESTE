...
async function execute(index) {
    ...
    const matches = await (0, selectMany_1.executeSelectMany)(this, index, dataStoreProxy);
    if (!dryRun) {
        const success = await dataStoreProxy.deleteRows(matches.map((x) => x.json.id));
        if (!success) {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `failed to delete rows for index ${index}`);
        }
    }
    return matches;
}
